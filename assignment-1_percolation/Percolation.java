...
public class Percolation {
    ...
    public void open(int i, int j) {      // open site (row i, column j) if it is not already -- appallingly we want 1<=i<=N
    ...
    public static void main(String[] args) {
        ...
        j++;
        StdOut.printf("If we open adjacent node [%d][%d], then they should be connected.\n", i, j);
        StdOut.printf("With [%d][%d] (%d) still closed, is it connected to [%d][%d] (%d)? ", i, j, something.grid[i][j], i, j-1, something.grid[i][j-1]);
        if ( !something.uf.connected( something.grid[i][j], something.grid[i][j-1] ) ) { StdOut.println("No!"); }
        else { StdOut.println("Yes, so something is wrong with my code."); }
        
        StdOut.printf("Now opening node [%d][%d] (%d)...", i, j, something.grid[i][j]);
        something.open(i, j);
        StdOut.println("done.");
        StdOut.printf("Now, is [%d][%d] (%d) connected to [%d][%d] (%d)? ", i, j, something.grid[i][j], i, j-1, something.grid[i][j-1]);
        
        if ( something.uf.connected( something.grid[i][j], something.grid[i][j-1] ) ) { StdOut.println("Yes!"); }
        else { StdOut.println("No, something is wrong with my code."); }
        
        StdOut.println();
        StdOut.printf("Is site [%d][%d] full? (It's full if it's connected to the top row by open sites.) ", i, j);
        if( something.isFull(i, j) ) { StdOut.println("Yes! Something has gone terribly wrong!"); }
        ...
        for ( count = 1; count < j; count++ ) {
            StdOut.printf("Site [%d, %d] (root %d)-- ", count, j, something.uf.find(something.grid[count][j]));
            something.open(count, j);
        }
        StdOut.println();
        
        StdOut.printf("\nSo now, is site [%d, %d] (root %d) full? ", i, j, something.uf.find(something.grid[i][j]));
        StdOut.println(something.isFull(i, j));
        
        StdOut.printf("What if we also open site [1, %d]? Then is [%d, %d] full? ", j, i, j);
        something.open(1, j);
        StdOut.println(something.isFull(i, j));
        
        // We say the system percolates if there exists a path from the top row to the bottom.
